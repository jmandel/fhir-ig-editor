...
// ---- engine ContentApi hook (TS-liquid sunset) -----------------------------
// The worker installs the wasm Session's content surface here before any
// render. Liquid evaluation happens IN THE ENGINE against the mounted cycle
// site tree (its own _includes + _data, tree-first); this module only supplies
// the tree and consumes html strings. markdown-it below is cycle's DESIGN
// layer (table-scroll wrappers, heading anchors, task lists), not a content
// engine — it stays TS with the React chrome.
export interface EngineContent {
  renderLiquid(source: string, dataJson: string): string;
  mountSite(filesJson: string, optionsJson: string): void;
}
let engineContent: EngineContent | null = null;
export function setEngineContent(c: EngineContent): void {
  engineContent = c;
}
function content(): EngineContent {
  if (!engineContent) throw new Error('engine content API not installed (setEngineContent)');
  return engineContent;
}

/** The byte-exact placeholder the M2 TS path rendered for includes that only
 *  exist in a full site build (generated by cycle's build wrapper). */
function missingIncludePlaceholder(name: string): string {
...
/** Mount cycle's include set + site data into the engine session (ONE call per
 *  site build): the project include registry's outputs, the site.db text
 *  assets, the M2 generated fragments, and the known build-wrapper-only
 *  includes as placeholders. Tree-first (engineFirstIncludes:false) so cycle's
 *  OWN include design wins over publisher fragment kinds of the same name. */
export function mountEngineSite(rows: SiteDbRows): void {
  const store = new RowStore(rows);
  setActiveStore(store);
  const ctx = new SiteContext(store);
  const files: Record<string, string> = {};
  // project/includes.ts registry outputs (dependency-table, ip-statements, …).
  for (const [name, gen] of Object.entries(includes)) {
    try {
      files[`_includes/${name}`] = (gen as (ig: unknown, params: Record<string, string>) => string)(ctx.igResource, {});
    } catch {
      // A registry generator that throws surfaces at include time instead.
    }
  }
  // site.db text assets are includable by name (cycle's assetInclude path).
  for (const a of store.assets()) {
    const text = store.textAsset(a.Name);
    if (text != null) files[`_includes/${a.Name}`] = text;
  }
  // M2 generated fragments + build-wrapper-only includes.
  const tp = ctx.generatedFragment('table-profiles.xhtml');
  if (tp != null) files['_includes/table-profiles.xhtml'] = tp;
  files['_includes/sample-viewer-links.md'] = missingIncludePlaceholder('sample-viewer-links.md');
  // site.data.* — the publisher-style liquid data (fhir.*).
  for (const [k, v] of Object.entries(ctx.liquidSiteData)) {
    files[`_data/${k}.json`] = JSON.stringify(v);
  }
  content().mountSite(JSON.stringify(files), JSON.stringify({ engineFirstIncludes: false }));
}
...
function renderNarrative(ctx: SiteContext, slug: string, body: string, title: string): string {
  // Engine-side Liquid (ContentApi): includes/site.data resolve against the
  // tree mountEngineSite() installed. cycle's {% sql %}/{% fragment %} tags are
  // SUNSET with TS liquid (its committed content uses neither — verified; use
  // would fail loud in the engine parser rather than silently vanish).
  let liquidOut = content().renderLiquid(body, '');
